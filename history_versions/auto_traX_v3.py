# + окно препятствия и окно расчета приземления
# max score 13605
    ...
    while True:
        ...
        if temp and np.abs(temp - _speed) < 2:
            ...
            if perf_counter() -  speed_timer > 15:
                speed_timer = perf_counter()
                x0 = int(base_obstacle_coords[0][0] * float(_speed/6))
                obstacle_coords = ((x0, base_obstacle_coords[0][1]), (x0 + 20, base_obstacle_coords[1][1]))
                safely_coords = ((x0 - 20, base_obstacle_coords[0][1]), (x0 + 20, base_obstacle_coords[1][1]))
